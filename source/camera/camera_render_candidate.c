...
/*   Updated: 2022/02/24 11:34:59 by javgonza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	render_segments_closest_to_farthest(t_camera *cam,
			t_collision_candidate *candidate, float *distances)
{
	size_t	i;
...
static float	aproximate_distance_to_segment_end(t_segment segment,
			t_vector pos, t_vector direction)
{
	float		start;
	float		end;
...
void	camera_render_candidate(t_camera *cam, size_t candidate_index)
{
	...
	dist_to_segments = malloc(sizeof(*dist_to_segments)
			* candidate->col->segment_count);
	...
	while (i < candidate->col->segment_count)
	{
		...
		dist_to_segments[i] = aproximate_distance_to_segment_end(
				segment_in_world, cam->pos, cam->direction);
		i++;
	}
