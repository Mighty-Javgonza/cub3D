...
/*   Updated: 2022/02/13 16:41:24 by javgonza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_wall_slice_interpolator	camera_get_interpolator_from_segment(t_camera *cam, t_collision_candidate *candidate, size_t segment_index)
{
	...
	start_col = collision_from_camera_pixel_with_segment(cam, segment_range.start + 1, &segment_as_seen_from_camera);
	if (start_col.exists)
	{
		start_col.target = candidate->col;
		start_col.target_id = segment_index;
	}
	end_col = collision_from_camera_pixel_with_segment(cam, segment_range.end - 1, &segment_as_seen_from_camera);
	if (end_col.exists)
	{
