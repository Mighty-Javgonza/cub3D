...
/*   Updated: 2022/02/20 17:15:42 by javgonza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calculate_slice_z_offset(t_wall_slice_painter *slice, t_camera *cam, t_collision col)
{
	...
	plane_bottom = (cam->z_axis_pos - camera_get_plane_height(cam) / 2);
	...
	slice->z_end_in_screen = (camera_get_res_y(cam) / 2) - (p * camera_get_res_y(cam) / 2);
	...
	slice->z_start_in_screen = (camera_get_res_y(cam) / 2) - (p * camera_get_res_y(cam) / 2) ;
