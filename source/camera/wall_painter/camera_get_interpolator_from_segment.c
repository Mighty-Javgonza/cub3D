...
/*   Updated: 2022/02/24 11:11:31 by javgonza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	clamp_range_to_representator_bounds(t_view_representator *vr,
...
static t_camera_range	get_biggest_segment_range_that_guarantees_collisions(
		t_camera *cam, t_segment segment, t_collision_candidate *candidate)
{
...
static t_collision	get_collision_with_target_from_segment_and_pixel(
		t_camera *cam, size_t pixel, t_collision_candidate *candidate,
		size_t segment_index)
{
	...
	segment = add_segment_vector(segment, candidate->col->pos);
...
t_wall_slice_interpolator	camera_get_interpolator_from_segment(t_camera *cam,
			t_collision_candidate *candidate, size_t segment_index)
{
	...
	segment = candidate->col->segments[segment_index];
	segment_range = get_biggest_segment_range_that_guarantees_collisions(cam,
			segment, candidate);
	clamp_range_to_representator_bounds(&cam->representator, &segment_range);
	interpolator.can_be_painted = 0;
	start_col = get_collision_with_target_from_segment_and_pixel(cam,
			segment_range.start, candidate, segment_index);
	end_col = get_collision_with_target_from_segment_and_pixel(cam,
			segment_range.end, candidate, segment_index);
	interpolator = interpolator_from_collisions(start_col, end_col,
			segment_range, cam);
	if (interpolator.can_be_painted
		&& !representator_check_area_has_unpainted_pixels(&cam->representator,
			interpolator.bound_box))
		interpolator.can_be_painted = 0;
