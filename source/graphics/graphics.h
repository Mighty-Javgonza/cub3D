...
/*   Updated: 2022/02/22 15:42:35 by javgonza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_graphic_image			new_graphic_image(t_graphic_environment *ge,
							t_pixpos res);
...
void					paint_bound_collider(t_graphic_image *img,
							t_bound_collider *col);
void					paint_ray_collider(t_graphic_image *img,
							t_ray_collider *rc);
void					paint_world(t_graphic_image *img, t_world *w);
void					put_pixel(t_graphic_image *img, t_vector pos,
							unsigned int color);
void					paint_rect(t_graphic_image *img, t_rectangle r);
void					clear_image(t_graphic_image *img);
t_graphic_image			graphic_image_from_png(t_graphic_environment *ge,
							char *path);
t_graphic_image			*new_image_from_png(t_graphic_environment *ge,
							char *path);
void					display_camera_view(t_graphic_environment *ge,
							t_camera *camera);
